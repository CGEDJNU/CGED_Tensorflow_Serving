...
class SpellChecker(object):
    ...
    def __init__(self,hps):
        ...
        #default model version equals 0
        self.builder_path = hps.log_root+'/'+hps.exp_name+'/export/0'
    ...
    def build_model_for_serving(self, sess):
        """
            builder_path: for example, 'model/export/0'
        """
        tfs_pinyin_ids      = tf.saved_model.utils.build_tensor_info(self.input_pinyin_ids)
        tfs_word_ids        = tf.saved_model.utils.build_tensor_info(self.input_word_ids)
        tfs_input_lengths   = tf.saved_model.utils.build_tensor_info(self.input_lengths)
        
        # get output value for dim by row(proba_prediction)
        tfs_pred            = tf.saved_model.utils.build_tensor_info(self.proba_prediction)
        
        # get keep_ratio
        tfs_keep_ratio   = tf.saved_model.utils.build_tensor_info(self.keep_ratio)

        pred_signature = tf.saved_model.signature_def_utils.build_signature_def(
                         inputs      = {'pinyin_ids': tfs_pinyin_ids, 'word_ids': tfs_word_ids, 'input_lengths': tfs_input_lengths, 'keep_ratio': tfs_keep_ratio},
                         outputs     = {'predict': tfs_pred},
                         method_name = tf.saved_model.signature_constants.PREDICT_METHOD_NAME)
        
        builder = tf.saved_model.builder.SavedModelBuilder(self.builder_path)
    ...
    def train_one_step(self, input_pinyin_ids,input_word_ids,input_lengths,target_word_ids, keep_ratio, sess):
    ...
    def infer(self, input_pinyin_ids,input_word_ids,input_lengths,target_word_ids, keep_ratio, sess):
        """perform inference"""
        feed_dict = {}
        feed_dict[self.input_pinyin_ids] = input_pinyin_ids
        feed_dict[self.input_word_ids] = input_word_ids
